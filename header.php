...
<body <?php body_class(); ?>>
	...
	<div class="mps-centered">

			<div class="xbutton"></div><!-- /xbutton -->

			<?php global $shortcode_tags; ?>
			<div class="mps-display-consult">
				<?php 	

					$val = 'mps popup form';
					$mpsContact = $shortcode_tags['contact-form-7'];

					if($mpsContact){
						$mpsShortCode = do_shortcode( '[contact-form-7 title="'.$val.'"]' ); 
						if($mpsShortCode === '[contact-form-7 404 "Not Found"]'){
							echo 'Please enter form title as "mps popup form"';
						}else{
							echo $mpsShortCode;
						}
					}else{
						echo "Error. Please Install 'contact-form-7' plugin.";
					}
									
				?>
			</div><!-- /mps-display-consult -->
			
			<div class="mps-display-optin">
				<?php 

					  $str = 'mps optin form';
					  $mpsOptin = $shortcode_tags['contact-form-7'];
					  
					  if($mpsOptin){
					  	$mpsOptin = do_shortcode( '[contact-form-7 title="'.$str.'"]' ); 
					  	if($mpsOptin === '[contact-form-7 404 "Not Found"]'){
					  		echo 'Please enter form title as "mps optin form"';
					  	}else{
					  		echo $mpsOptin;
					  	}
					  }else{
